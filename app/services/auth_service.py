...
class AuthService : 
    ...
    login_data: LoginSchema
        ...
        try:
            ...
            if result is None:
                logger.warning(
                    f"login failed for username {login_data.user_name}"
                )
                return None

            logger.info(
                f"user {login_data.user_name} logged in successfully"
            )
            ...
            logger.error(
                f"login error for username {login_data.user_name}: {e}"
            )
            print (e)
            raise InternalServerException(
    ...
    refresh_token: str
        ...
        try:
            result = self.repo.refresh_access_token(
                db,
                refresh_token
            )
