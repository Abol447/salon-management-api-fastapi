...
def CreateCustomer(
    ...
    return service.CreateCustomer(db=db, customer_data=customer_data)
...
def GetMyCustomer(
    ...
    return service.get_by_user_id(db, int(user["sub"]))
...
def UpdateCustomer(
    ...
    return service.update(db, customer_data, int(user["sub"]))


@router.delete("", response_model=CustomerResponse)
def DeleteCustomer(
    user: dict = Depends(get_current_user),
    db: Session = Depends(get_db),
    service: CustomerService = Depends(GetCustomerService),
):
    return service.delete(db, int(user["sub"]))
